...
export const authService = {
  async login(email: string, password: string): Promise<AuthLoginResponse> {
    if (!AUTH_USE_BACKEND) {
      throw new Error("Backend auth disabled");
    }

    const res = await apiClient.post<AuthLoginResponse>("/v1/auth/login", {
      email,
      password,
    });
    return res.data;
  ...
  async getMe(): Promise<AuthUserProfile> {
    const res = await apiClient.get<AuthUserProfile>("/v1/auth/me");
    return res.data;
  },
};
