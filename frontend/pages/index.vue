  ...
  <main class="min-h-screen bg-[#0d0f10] text-white">
    ...
    <template v-else>
      ...
      <section class="mx-auto max-w-[1280px] px-6 py-20 md:px-12">
        <div class="flex items-end justify-between"><div><p class="eyebrow">Mở bán tại {{ selectedBranch?.name || 'hệ thống' }}</p><h2 class="section-title">Phim có suất để đặt ngay</h2></div><NuxtLink to="/phim" class="more-link">Xem tất cả →</NuxtLink></div>
      ...
      <section class="mx-auto max-w-7xl px-5 py-16 md:px-10"><div class="flex items-end justify-between"><div><p class="eyebrow">Sắp ra mắt</p><h2 class="section-title">Phim sắp chiếu</h2><p v-if="followedMovieIds.length" class="mt-2 text-xs text-pink-300">Đang theo dõi {{ followedMovieIds.length }} phim trên thiết bị này.</p></div><NuxtLink :to="{ path: '/phim', query: { status: 'UPCOMING' } }" class="more-link">Xem toàn bộ →</NuxtLink></div><div v-if="upcomingMovies.length" class="mt-8 grid grid-cols-2 gap-5 md:grid-cols-4"><div v-for="movie in upcomingMovies" :key="movie.id" class="relative"><ProductCard v-bind="movie" /><button class="absolute right-3 top-3 z-20 flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-black/70 backdrop-blur" :class="isFollowed(movie.id) ? 'text-pink-400' : 'text-white'" :title="isFollowed(movie.id) ? 'Bỏ theo dõi phim' : 'Theo dõi phim'" @click="toggleFollow(movie.id)"><span class="material-symbols-outlined" :style="isFollowed(movie.id) ? `font-variation-settings:'FILL' 1` : ''">favorite</span></button></div></div><div v-else class="empty">Không có phim sắp chiếu phù hợp.</div></section>
