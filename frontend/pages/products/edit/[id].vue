...
<template>
  <div v-if="!product" class="py-16 text-center text-on-surface-variant">
    ...
    <NuxtLink
      to="/products"
      class="text-primary-container underline block mt-2"
    >
      ← Quay lại danh sách
    </NuxtLink>
  </div>
  ...
  >
    <NuxtLink
      to="/products"
      class="inline-block mb-6 text-sm text-on-surface-variant hover:text-primary-container"
    >
      ← Quay lại danh sách
    </NuxtLink>
    ...
    <div class="grid md:grid-cols-2 gap-10">
      <img
        ...
        :alt="product.name"
        class="w-full rounded-2xl object-cover aspect-[2/3]"
      />

      <div>
        <span class="text-xs font-bold uppercase tracking-wide text-primary-container">
          {{ product.category }}
        </span>

        <h1 class="font-headline-lg text-3xl font-bold text-on-surface mt-2 mb-4">
          {{ product.name }}
        </h1>
