...
<template>
    ...
    <section class="py-16 max-w-container-max mx-auto px-6 md:px-margin-desktop">
        ...
        <div v-if="items.length === 0" class="text-center py-10 text-on-surface-variant">
            <div v-if="orderId"
                class="rounded-2xl border border-primary-container/20 bg-primary-container/10 p-6 max-w-xl mx-auto">
                <p class="text-lg font-semibold text-on-surface">Đặt hàng thành công!</p>
                <p class="mt-2">Mã đơn hàng của bạn là <span class="font-bold text-primary-container">{{ orderId
                        }}</span>.</p>
        ...
        <template v-else>
            <div class="space-y-4 mb-8">
                <div v-for="item in items" :key="item.id"
                    class="flex items-center gap-4 bg-surface-container-low border border-glass-stroke rounded-xl p-4">
                    ...
                    <p class="font-bold text-on-surface">{{ new Intl.NumberFormat('vi-VN').format(item.price * item.qty
                        * 1000) }}₫</p>
            ...
            <div
                class="flex flex-col gap-4 border-t border-glass-stroke pt-6 md:flex-row md:items-center md:justify-between">
                <div>
                    <p class="text-xl font-bold text-on-surface">Tổng: {{ new Intl.NumberFormat('vi-VN').format(total *
                        1000) }}₫</p>
                    <p class="text-sm text-on-surface-variant mt-1">Đơn hàng sẽ được xác nhận sau khi bạn nhấn thanh
                        toán.</p>
                </div>
                <div class="flex gap-3">
                    <NuxtLink to="/products"
                        class="px-6 py-3 rounded-xl border border-glass-stroke hover:bg-white/10 transition">Tiếp tục
                        mua sắm</NuxtLink>
                    <button @click="checkout" :disabled="isCheckingOut"
                        class="bg-primary-container text-on-primary-container px-6 py-3 rounded-xl font-bold red-glow-hover disabled:opacity-60 disabled:cursor-not-allowed">
